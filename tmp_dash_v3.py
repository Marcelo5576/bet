...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .nav-link:hover {{ background: var(--nav-hover-bg); border-color: #3b4859; color: var(--amber); }}
    ...
    .match-visual {{
      background: radial-gradient(circle at 18% 22%, rgba(91, 140, 255, .16), transparent 34%), linear-gradient(140deg, #151a22 0%, #0e141c 100%);
    ...
    body[data-theme='light'] .stats-tab {{ color: #5c6675; }}
    body[data-theme='light'] .stats-tab.active {{ color: #111827; }}
    body[data-theme='light'] .match-avatar {{
      background: #f1f5fb;
      border-color: #cfdaea;
      color: #1b2c45;
    }}
    body[data-theme='light'] .match-visual-track {{
      background: #e9f0fb;
      border-color: #d5e0ee;
    }}
...
async def api_simulator_session(
    ...
    if not prepared:
        prepared = _simulation_signals(state)
...
def _simulation_session_panel(session: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"<div class='sim-lab-kpi'><div class='muted'>Banca inicial</div><div class='metric'>{_esc(session.get('start_bankroll', 0))}u</div></div>"
        f"<div class='sim-lab-kpi'><div class='muted'>Banca final</div><div class='metric {_value_class(session.get('profit_units', 0))}'>{_esc(session.get('end_bankroll', 0))}u</div></div>"
        f"<div class='sim-lab-kpi'><div class='muted'>Lucro</div><div class='metric {_value_class(session.get('profit_units', 0))}'>{_esc(session.get('profit_units', 0))}u</div></div>"
        f"<div class='sim-lab-kpi'><div class='muted'>Max drawdown</div><div class='metric red'>{_esc(session.get('max_drawdown', 0))}u</div></div>"
...
def _simulation_history_panel(sessions: list[dict[str, Any]]) -> str:
    ...
    for idx, session in enumerate(sessions[:40], start=1):
        ...
        rows.append(
            ...
            f"<td class='{_value_class(profit)}'>{_esc(round(profit, 2))}u</td>"
            f"<td>{_esc(round(end_bankroll, 2))}u</td>"
            f"<td class='red'>{_esc(round(drawdown, 2))}u</td>"
            "</tr>"
        )
    return (
...
def _sim_result_rows_html(rows: list[dict[str, Any]]) -> str:
    ...
    for row in rows:
        ...
        html_rows.append(
            ...
            f"<td>{_esc(row.get('stake'))}u</td>"
            f"<td class='{outcome_cls}'>{_esc(outcome)}</td>"
            f"<td class='{_value_class(row.get('profit'))}'>{_esc(row.get('profit'))}u</td>"
            f"<td>{_esc(row.get('bankroll'))}u</td>"
            "</tr>"
        )
